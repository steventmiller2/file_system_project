...
public class SQLiteFileSystemDAO implements FileSystemDAO {
    ...
    private boolean hasPermission(String tableName, String entryName, int parentDirectoryId) {
        if (userSession.getLoggedInUser().isAdmin()) {
            ...
            String selectPermissionSQL = "SELECT permissions, owner_id FROM " + tableName + " WHERE name = ? AND parent_directory_id = ?;";
    ...
    @Override
    public void updatePermissions(String entryName, int permissions, int parentDirectoryId) {
        String updatePermissionsFileSQL = "UPDATE file SET permissions = ? WHERE name = ? AND parent_directory_id = ? AND (owner_id = ? OR ?);";
        String updatePermissionsDirSQL = "UPDATE directory SET permissions = ? WHERE name = ? AND parent_directory_id = ? AND (owner_id = ? OR ?);";
        ...
        try (PreparedStatement stmtFile = connection.prepareStatement(updatePermissionsFileSQL);
            ...
            stmtFile.setInt(4, userSession.getLoggedInUser().getUserId());
            stmtFile.setInt(5, userSession.getLoggedInUser().isAdmin() ? 1:0);
            ...
            if (affectedRowsFile == 0) {
                ...
                stmtDir.setInt(4, userSession.getLoggedInUser().getUserId());
    ...
    public Directory getChildDirectory(String dirName, int currentDirectoryId) {
        String selectDirectorySQL = "SELECT * FROM directory WHERE name = ? AND parent_directory_id = ?;";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(selectDirectorySQL)) {
            ...
            if (resultSet.next()) {
                ...
                int owner_id = resultSet.getInt("owner_id");
                int permissions = resultSet.getInt("permissions");
                return new Directory(dirName, currentDirectoryId, id, owner_id, permissions);
            } else {
    ...
    public Directory getParentDirectory(int parentDirectoryId) {
        String selectDirectorySQL = "SELECT * FROM directory WHERE id = ?;";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(selectDirectorySQL)) {
            ...
            if (resultSet.next()) {
                ...
                int permissions = resultSet.getInt("permissions");
                return new Directory(name, parentDirId, id, ownerId, permissions);
            } else {
                System.err.println("Directory not found.");
            }
        } catch (SQLException e) {
    ...
    public FileSystemEntry readFile(String fileName, int parentDirectoryId) {
        String selectFileSQL = "SELECT * FROM file WHERE name = ? AND parent_directory_id = ?;";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(selectFileSQL)) {
            ...
            if (resultSet.next()) {
                ...
                int ownerId = resultSet.getInt("owner_id");
                int permissions = resultSet.getInt("permissions");
                return new File(fileName, content, parentDirectoryId, ownerId, permissions);
            } else {
    ...
    public List<FileSystemEntry> getFilesInDirectory(int directoryId) {
        ...
        String selectFilesSQL = "SELECT * FROM file WHERE parent_directory_id = ?;";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(selectFilesSQL)) {
            ...
            while (resultSet.next()) {
                ...
                int ownerId = resultSet.getInt("owner_id");
                int permissions = resultSet.getInt("permissions");
                files.add(new File(name, content, parentDirectoryId, ownerId, permissions));
            }
        } catch (SQLException e) {
    ...
    public List<FileSystemEntry> getSubDirectories(int directoryId) {
        ...
        String selectSubDirectoriesSQL = "SELECT * FROM directory WHERE parent_directory_id = ?;";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(selectSubDirectoriesSQL)) {
            ...
            while (resultSet.next()) {
                ...
                int ownerId = resultSet.getInt("owner_id");
                int permissions = resultSet.getInt("permissions");
                subDirectories.add(new Directory(name, parentDirectoryId, id, ownerId, permissions));
            }
        } catch (SQLException e) {
